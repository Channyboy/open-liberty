...
class TimerAdapter implements org.eclipse.microprofile.metrics.Timer, MeterHolder {
    ...
    public TimerAdapter register(MpMetadata metadata, MetricDescriptor descriptor, String scope, Tag... globalTags) {
        System.out.println("timer debug 1: metric " + metadata.getName());
        if (globalCompositeTimer == null || metadata.cleanDirtyMetadata()) {
            ...
            System.out.println("timer debug 2: percentiles value config" + percentilesConfig);
            ...
            Builder builder = Timer.builder(descriptor.name()).description(metadata.getDescription()).tags(tagsSet).percentilePrecision(PRECISION);
            ...
            if (defaultBucketConfig != null && defaultBucketConfig.isEnabled()) {
                ...
                TimerBucketMinConfiguration defaultBucketMinConfig = MetricsConfigurationManager.getInstance().getDefaultTimerMinBucketConfiguration(metadata.getName());
