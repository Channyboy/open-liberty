...
public class PrometheusMetricsExporter implements Exporter {
    ...
    public PrometheusMetricsExporter() {
        ...
        prometheusRegistryList = Metrics.globalRegistry.getRegistries().stream().filter(registry -> registry instanceof PrometheusMeterRegistry).collect(Collectors.toList());
...
//        Set<String> scrapeMeterNames = new HashSet<String>();
//        String promName = resolvePrometheusName(metricName);
//        String stripped = stripEndingIfCounter(promName);
//        if (stripped != null) {
//            System.out.println("Adding : " + stripped);
//            scrapeMeterNames.add(stripped);
//        } else {
//            System.out.println("Adding : " + promName);
//            scrapeMeterNames.add(promName);
//        }

        Set<String> scrapeMeterNames = calculateMeterNamesToScrape(metricName, unitTypesSet);
//        System.out.println("debug 3 : " + scrapeMeterNames);

        // Strip #EOF from output
        String scrapeOutput = prometheusMeterRegistry.scrape(TextFormat.CONTENT_TYPE_004, scrapeMeterNames).replaceFirst("\r?\n?# EOF", "");

//        for (Meter m : prometheusMeterRegistry.getMeters()) {
//            System.out.println("look at tihs " + m.getId());
//        }

//        System.out.println("debug 4 " + scrapeOutput);
    ...
    private String stripEndingIfCounter(String meterName) {
        String ret = null;
        if (meterName.endsWith("_total")) {
            ret = meterName.substring(0, meterName.length() - "_total".length());
            System.out.println("Stripped is " + ret);
        }
        return ret;
    }

    private Set<String> calculateMeterNamesToScrape(String name, Set<String> unitTypeSet) {
        ...
        for (String unit : unitTypeSet) {
            System.out.println("Adding this: " + promName + unit);
            retSet.add(promName + unit);
        }
        if (LOGGER.isLoggable(Level.FINEST)) {
...
//        System.out.println("Adding this name : " + "REST.request.unmappedException");
//        retSet.add("REST.request.unmappedException");
//        System.out.println("Adding this name : " + "REST_request_unmappedException");
//        retSet.add("REST_request_unmappedException");

        for (String unit : unitTypeSet) {
            for (String suffix : meterSuffixSet) {
                System.out.println("Adding this: " + promName + unit + suffix);
