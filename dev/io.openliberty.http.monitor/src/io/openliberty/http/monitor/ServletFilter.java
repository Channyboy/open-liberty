...
public class ServletFilter implements Filter {
		...
		try {
			...
			if (httpRoute == null) {
				...
				if (servletRequest instanceof SRTServletRequest) {
					...
					System.out.println("\n################################## --------------- ");
					System.out.println("srtservReqeust PathInfo: " + pathInfo);
					System.out.println("srtservReqeust ServletPath: " + servletPath);
					System.out.println("srtservReqeust contxtpath: " + contextPath);
					System.out.println("srtservReqeust getRequestURI: " + fullRequestURI);
					System.out.println("----------------------------------------------- ");
					
					
					/*
					 * path Info is null (no extrra path info) and servlet path is not null
					 */
					if (pathInfo == null && servletPath != null && !servletPath.isEmpty()) {
						//TODO: double check that it starts with /?
						nhp = contextPath + servletPath;
					} 
					/*
					 * Special- like /metrics or /health
					 * PathInfo is "/" 
					 */
					else if (pathInfo == "/" && servletPath != null && servletPath.isEmpty()) {
						nhp = fullRequestURI;
					}
					
					//no idea what it is
					else if (servletPath != null  && servletPath.isEmpty() && pathInfo != null && pathInfo != "/" && !pathInfo.isEmpty()) {
						nhp = contextPath  + "/*";
					}
					
					System.out.println("the NEW http route is resolved to ---> " + nhp);
					
					
					httpRoute = oldResolveHttpRoute(httpRoute, srtServletRequest, contextPath, httpStatsAttributesHolder);
				}
	...
	private String oldResolveHttpRoute(String httpRoute, SRTServletRequest srtServletRequest, String contextPath, HttpStatAttributes httpStatsAttributesHolder) {
		...
		System.out.println("(Existing algo) resolved the httpRoute to " + httpRoute);
		return httpRoute;
	}
